...
using UnityEngine;
using PHFramework;
using PHFramework.Draw;
using PHFramework.Entity;
using PHFramework.Event;
using PHFramework.Fsm;
using PHFramework.Resource;
using PHFramework.Console;
...
public class Game : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        {
            PHConsole.IsOpen = !PHConsole.IsOpen;
        }

        string buttonName = PHConsole.IsHunterEnable ? "关闭Hunter" : "打开Hunter";
        if (GUI.Button(new Rect(120, Screen.height - 60, 100, 50), buttonName))
        {
            if (PHConsole.IsHunterEnable)
            {
                PHConsole.Excute("disable_hunter");
            }
            else
            {
                PHConsole.Excute("enable_hunter");
            }
        }
    }
